...
class jqueryRte_viewHelper extends  jQueryPlugin{
	...
	function init(){
		$iframeSrc = $this->view->url('uploadform','docs',array('embed'=>"'+inputId+'"),true);
		/*if(FRONT_NAME !== 'admin'){
			#- ~ ".(FRONT_NAME!=='admin'?"buttonSet: 'bold|underline|italic|remove|toggle',":'')."
			jQuery::onReadyDocument("
			$.extend($.fn.rte.defaults,{
				imgPath:'".ROOT_URL.jQuery::$pluginPath."/jqueryRte/',
				css_url:'".ROOT_URL."/styles/defaut.css',
				buttonSet: 'bold|underline|italic|remove|toggle'
			});
			");
		}else{*/
			jQuery::onReadyDocument("
	...
	*                             for exemple areaEditor('myArea',array('value'=>'my area content','style'=>'border:solid silver 1px'));
	...
	*  @return string.
	*/
	function jqueryRte($areaSelector,$areaOptions=null){
